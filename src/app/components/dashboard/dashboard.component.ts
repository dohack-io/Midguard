...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.taskService.getTaskAnnouncer()
      .pipe(takeUntil(this.$destroy))
      .subscribe(message => this.messageService.add(
        {severity:'success', summary:message, detail:this.taskService.getTaskById(this.user.taskId).name}
      ));
    this.userService.getUserAnnouncer()
      .pipe(takeUntil(this.$destroy))
      .subscribe(message => this.messageService.add(
        {severity:'success', summary: message.toString(), detail:this.taskService.getTaskById(this.user.taskId).name}
      ));
    this.inventoryService.getInventoryAnnouncer()
      .pipe(takeUntil(this.$destroy))
      .subscribe(message => this.messageService.add(
        {severity:'success', summary: message.toString()}
      ));
  }
  ...
  returnToOverview() {
    ...
    if(this.router.url == '/dashboard/crackCode' ||
  ...
  search(): void {
    this.displaySB = false;
    this.router.navigate(['/dashboard/profile/', this.userService.getUserIdByName(this.searchStr)]);
  }
