...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params
      .pipe(takeUntil(this.$destroy), filter(params => params.id), switchMap(params => params.id))
      .subscribe((id: number) => {
        this.user = this.userService.getUserById(id);
        for(let id of this.user.profileItems) {
  ...
  sendMessage() {
    let chatId = this.chatService.startNewConversation(this.userService.getUser().id, this.user.id);
  ...
  removeProfileItem(item: Item) {
    ...
    this.profileItems.splice(this.user.profileItems.indexOf(item.id),1);
  }

  openAddItemDialog() {
    this.allItems = this.inventoryService.getAllItems();
    this.allItems = this.allItems.filter(item => this.user.profileItems.indexOf(item.id) < 0);
